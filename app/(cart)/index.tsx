...
const generateTimeSlots = () => {
  ...
  for (let hour = 8; hour <= 21; hour++) {
    for (let min = 0; min < 60; min += 30) {
      ...
      slots.push(
        `${displayHour.toString().padStart(2, '0')}:${formattedMin} ${period}`
      );
    }
  }
  return slots;
...
export default function CartScreen() {
  ...
  const { orders, updateOrder, removeOrder, moveToSaveLater, refreshCart } =
    useCart();
  ...
  const [orderStatus, setOrderStatus] = useState<null | 'loading' | 'success'>(
    null
  );
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Error', 'Failed to remove order. Please try again.');
  ...
  const handleMoveToSaveLater = (orderId: string) => {
    Alert.alert('Save for Later', 'Move this item to save for later?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Move',
        onPress: async () => {
          try {
            await moveToSaveLater(orderId);
            // Clean up associated data
            setOrderProblems((prev) => {
              const updated = { ...prev };
              delete updated[orderId];
            ...
            Alert.alert('Success', 'Item moved to save for later');
          } catch (error) {
      ...
      try {
        ...
        if (order) {
          ...
          if (selectedProblems.length > 0) {
            formattedMessage = `(${selectedProblems.join(', ')})`;
          }
          if (message) {
        ...
        Alert.alert(
          'Warning',
          'Problems saved locally but failed to sync with server'
        );
      }
    }
  ...
  const handleSaveSchedule = async () => {
    ...
    if (activeOrderId) {
      ...
      try {
        const order = orders.find((o) => o.orderId === activeOrderId);
        if (order) {
          // Get the existing message from state
          const problems = orderProblems[activeOrderId] || [];
          const customMessage = orderMessages[activeOrderId] || '';
        ...
        console.error('Error updating cart with schedule:', error);
        Alert.alert(
          'Warning',
          'Schedule saved locally but failed to sync with server'
        );
      }
    }
  ...
  const handleConfirmBooking = async () => {
    ...
    for (const order of orders) {
      ...
      if (!orderSchedules[order.orderId]) {
        Alert.alert(
          'Error',
          `Please select schedule for ${order.mainService.name}`
        );
        return;
      }
    }
    ...
    try {
      // Cart items are already updated in database when users clicked "Done" in modals
      // Backend will fetch the updated cart items from database
      const payload = {
      ...
      if (response && response.status) {
        // Clear local meta data (problems, messages, schedules)
        await clearAllCartMeta();

        // Refresh cart from backend to update state
        await refreshCart();
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
        ...
        {orders.length === 0 ? (
          ...
          <>
            ...
            {orders.map((order: any) => {
              ...
              const hasSchedule =
                orderSchedules[order.orderId]?.date &&
                orderSchedules[order.orderId]?.time;
                  ...
                  >
                    ...
                    <View style={styles.orderHeaderRight}>
                      <TouchableOpacity
                        onPress={(e) => {
                          e.stopPropagation();
                          handleMoveToSaveLater(order.orderId);
                        }}
                              ...
                              onPress={() => {
                                ...
                                if (currentQty === 1) {
                                  ...
                                  if (updatedServices.length === 0) {
                                    ...
                                    const newTotalQty = updatedServices.reduce(
                                      (sum: number, s: any) =>
                                        sum + parseInt(s.qty),
                                      0
                                    );
                                    const newTotalPrice =
                                      updatedServices.reduce(
                                        (sum: number, s: any) =>
                                          sum +
                                          parseInt(s.price) * parseInt(s.qty),
                                        0
                                      );
                                  ...
                                  const newTotalPrice = updatedServices.reduce(
                                    (sum, s) =>
                                      sum + parseInt(s.price) * parseInt(s.qty),
                                    0
                                  );
                                  updateOrder(order.orderId, {
                                    services: updatedServices,
            ...
            <View style={styles.noticeCard}>
              ...
              <Text style={styles.notice}>
                <Text style={{ fontWeight: '700' }}>
                  Visit & Diagnosis charge ₹100.
                </Text>{' '}
                That is applicable only if the service is denied by the customer
                after the serviceman's visit to the service location.
              </Text>
            </View>
            <TouchableOpacity
              activeOpacity={0.8}
      ...
      {showProblemModal &&
        ...
        (() => {
          const order = orders.find((o) => o.orderId === activeOrderId);
          const currentProblems = order
            ? problemOptions[order.mainService.slug] || []
            : [];
